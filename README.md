...
- `model/gnn_models.py`: Contains the core PyTorch implementations of the Graph Neural Networks, including the Dimension-level Risk Perception Module (DRPM), Long-hop Risk Perception Module (LRPM), and the HMARN architecture.
- `model/calibration_model.py`: The implementation of the LLM-based calibration method (`model_calibration` class) used to align and fuse the outputs from the GNN models and the LLM Multi-Agent system.
- `prompt/llm_prompt.py`: Contains the carefully designed prompt templates and agent roles (finance, ceo, news, laws, vios) for the multi-agent system.
...
- `data/enterprise_data.csv`: A desensitized subset of the enterprise dataset containing key features (e.g., Total Assets, Cash Ratio, Operating Profit Margin), violation labels, and node indices (`graph_id`).
- `data/graph_triple.nt`: The topological structure data represented as relational triples (subject, predicate, object) used to construct the relational graphs.
